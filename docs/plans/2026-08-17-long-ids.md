# `long` som id — besluttet, planlagt, endnu uden skivenummer
...
> **Målt om 2026-08-17, efter tre leverancer.** Planen blev skrevet som skive 8 og lagt til side
> samme dag. Siden er Alt-genvejene (som overtog nummer 8), Swagger-linket og skive 9 leveret, så
> tallene i planen var forældede. De er rettet, og **væksten er værd at kende, fordi den er
> planens eget argument**: id-aftrykket voksede fra **11 til 12** steder i tests, `task-list.spec.ts`
> fik en **femte** Guid-fixture, og skive 9 lagde en `PutAsync(Guid id, object body)`-hjælper til
> i `TaskEndpointsTests`. Tre steder over tre leverancer.
>
> **Det er reelt, men beskedent — og det svækker hastværks-argumentet.** Planen siger, at skiven
> *"bliver dyrere for hver skive der lægges imellem"*, og det er sandt; men målt er taksten cirka
> ét sted per leverance, ikke en eksplosion. Beslutningen bør derfor hvile på ergonomien —
> at "opgave 42" kan siges højt — frem for på en stigende regning. Bliver den udskudt igen, er
> det stadig værd at spørge hvorfor, men ikke fordi klokken tikker hurtigt.
>
> Baseline er nu **38 Core, 115 Api, 25 E2E, 143 Vitest**.


Skrevet som skive 8 den 2026-08-17 og derefter **bevidst udskudt** samme dag; Alt-genvejene overtog nummeret, så **ingen skive blev omnummereret**. Planen er færdig og målt igennem — den mangler kun en plads i rækkefølgen. Argumentet for at tage den snart står under "Og bemærk prisen" i `docs/HANDOFF.md`.
...
| Tests, **id-relateret** | **12** i 5 filer |
| **Builders** | **0** |
...
**"Næsten hver test" er også for stærkt.** Af de 15 `Guid`-forekomster i tests er 3 slet ikke id'er: `RunningHost` bruger `Guid.NewGuid()` til et midlertidigt filnavn, og `DatabaseBackupTests` til en midlertidig mappe og en unik titel. De skal **ikke** røres.
...
| --- | --- | --- |
| `Tasks` | `Id`, `SourceId`, `Title`, `Note`, `Deadline`, `Requester`, `ExternalKey`, `Status`, `WaitingOn`, `WaitingSince`, `DeferUntil`, `CompletedAt`, `CreatedAt` | `IX_Tasks_Deadline`, `IX_Tasks_SourceId_ExternalKey` |
...
> **Kolonnelisten vokser, og migreringen nævner hver kolonne ved navn.** `DeferUntil` kom med
> skive 9 og er tilføjet ovenfor og i SQL'en i Task 3 — men havde nogen kørt planen uden at
> måle om, ville den håndskrevne `CREATE TABLE`/`INSERT` have **droppet kolonnen i tavshed**.
> Det er præcis den fejl designdokumentets afsnit 10 advarer om: en SQLite-migrering der
> fjerner en kolonne taber dens data uden at sige noget.
>
> **Sammenlign derfor listen med `TodoDbContextModelSnapshot.cs` før du kører Task 3**, og hold
> `Up`, `Down` og `SELECT`-listerne i takt. En kolonne der er kommet til siden 2026-08-17 står
> ikke i denne plan.
...
De 12 id-relaterede forekomster. Mønsteret er overalt det samme: en hårdkodet Guid-streng eller et `Guid.NewGuid()` brugt som "et id der ikke findes" bliver et tal.

- `Guid.NewGuid()` som "findes ikke" → et tal der ikke findes, fx `999999`.
- En hårdkodet Guid i en URL → tallet.
- `Guid.Empty` → `0`.
- **`TaskEndpointsTests.PutAsync(Guid id, object body)`** → `long`. Den private hjælper kom med skive 9 og er den ene forekomst der er lagt til siden planen blev skrevet.
...
Forventet: **38 Core, 115 Api, 25 E2E** — grønne igen. Alle tests kører mod en frisk midlertidig database, hvor migreringen bare opretter tomme tabeller, så det her beviser at skemaet passer til modellen — **ikke** at data overlever. Det er Task 4.
...
Forventet: **116 Api** (115 + denne), 38 Core, 25 E2E. Rapportér de faktiske tal.
...
- **`task-list.spec.ts`** har **fem** Guid-strenge (`'11111111-…'` til `'55555555-…'` — den femte kom med skive 9's udskudte opgave), to underopgave-id'er `'aaaa'` og `'bbbb'`, og ét `id: 'x'` i en enkelt test. Alle bliver tal. Vælg `1`–`5` til opgaverne og fx `11`, `12` til underopgaverne, så det er til at se hvad der er hvad; `'x'` skal bare være et id der ikke kolliderer med de andre.
...
Forventet: bygningen lykkes, **143 Vitest**. Et andet tal betyder at en test er tabt eller duplikeret.
...
Og opdatér **Testtal** med de tal du **målte**. Forventet: 38 Core, **116** Api, 25 E2E, 143 Vitest. Planen har **intet skivenummer** — se rettelsen øverst — så skriv den ind som de øvrige leverancer uden for skiverne frem for som "Efter skive N".
...
- 38 Core, 116 Api, 25 E2E, 143 Vitest — grønne, og tallene skrevet ned som målt.
